<template>
  <div class="comment">
    <div
      class="card shadow border-success my-1"
      style="background-color:#ddd;minWidth:35em"
    >
      <div class="d-flex justify-content-between px-2">
        <span class="text-left">
          <!-- TASK ID -->
          <div class="text-muted text-left" :style="{ fontSize: fontSize }">
            {{ comment.taskId }}
          </div>
        </span>
        <span>
      ...
      <div class="card-body p-2 text-wrap">
        ...
        <span class="d-flex justify-content-between">
          <div class="text-success text-left" :style="{ fontSize: fontSize }">
            {{ comment.id }}
          </div>
          <div class="text-right" :style="{ fontSize: fontSize }">
...
export default {
  ...
  methods: {
    ...
    deleteComment() {
      swal({
        title: "Are you sure?",
        text: "Click 'Ok' to confirm you wish to delete Comment",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      }).then((willDelete) => {
